...
build: clean
	...
	CGO_ENABLED=0 go build -v -trimpath -ldflags '-X "github.com/traefik/neo-agent/pkg/version.date=${BUILD_DATE}" -X "github.com/traefik/neo-agent/pkg/version.version=${VERSION}" -X "github.com/traefik/neo-agent/pkg/version.commit=${SHA}"' -o ${OUTPUT} ${MAIN_DIRECTORY}
