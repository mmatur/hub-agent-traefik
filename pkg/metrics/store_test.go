...
func TestStore_Insert(t *testing.T) {
	...
	var got []DataPoint
	store.ForEach("1m", func(edgeIngr, ingr, svc string, pnts DataPoints) {
		got = append(got, pnts...)
	})
...
func TestStore_RollUp(t *testing.T) {
	...
	store.ForEach("10m", func(edgeIngr, ingr, svc string, pnts DataPoints) {
		assert.Len(t, pnts, 11)
	})

	store.ForEach("1h", func(edgeIngr, ingr, svc string, pnts DataPoints) {
		assert.Len(t, pnts, 2)
	})
}
